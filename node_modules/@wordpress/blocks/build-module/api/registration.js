 ...
 */
import { camelCase, isFunction, isNil, isPlainObject, mapKeys, omit, pick, pickBy, some } from 'lodash';
/**
...
export function unstable__bootstrapServerSideBlockDefinitions(definitions) {
  for (var _i = 0, _Object$keys = Object.keys(definitions); _i < _Object$keys.length; _i++) {
    ...
    serverSideBlockDefinitions[blockName] = mapKeys(pickBy(definitions[blockName], function (value) {
      return !isNil(value);
    }), function (value, key) {
...
export function registerBlockType(name, settings) {
  ...
  if (settings.deprecated) {
    settings.deprecated = settings.deprecated.map(function (deprecation) {
      ...
      // present don't fail.
      _objectSpread(_objectSpread({}, omit(preFilterSettings, DEPRECATED_ENTRY_KEYS)), deprecation), name), DEPRECATED_ENTRY_KEYS);
    });
  }
...
export function unregisterBlockType(name) {
  var oldBlock = select(blocksStore).getBlockType(name);
