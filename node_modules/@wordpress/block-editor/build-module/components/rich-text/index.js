...
function addActiveFormats(value, activeFormats) {
  if (activeFormats.length) {
    ...
    while (index--) {
      value.formats[index] = [].concat(_toConsumableArray(activeFormats), _toConsumableArray(value.formats[index] || []));
    }
  }
}
/**
...
function RichTextWrapper(_ref2, forwardedRef) {
  ...
  var nativeProps = useNativeProps();
  ...
  var splitValue = useCallback(function (record) {
    ...
    var isAfterOriginal = isEmpty(before) && !isEmpty(after); // Create a block with the content before the caret if there's no pasted
  ...
  var mergedRef = useMergeRefs([forwardedRef, fallbackRef]);
  var content = createElement(RichText, _extends({
    clientId: clientId,
