...
export default function useInnerBlockTemplateSync(clientId, template, templateLock, templateInsertUpdatesSelection) {
  var getSelectedBlocksInitialCaretPosition = useSelect(function (select) {
    return select(blockEditorStore).getSelectedBlocksInitialCaretPosition;
  }, []);
  ...
  var existingTemplate = useRef(null);
  useLayoutEffect(function () {
    ...
    if (innerBlocks.length === 0 || templateLock === 'all') {
      var hasTemplateChanged = !isEqual(template, existingTemplate.current);

      if (hasTemplateChanged) {
        existingTemplate.current = template;
        var nextBlocks = synchronizeBlocksWithTemplate(innerBlocks, template);
