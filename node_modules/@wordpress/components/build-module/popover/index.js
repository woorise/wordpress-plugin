...
import { useViewportMatch, useResizeObserver, useFocusOnMount, __experimentalUseFocusOutside as useFocusOutside, useConstrainedTabbing, useFocusReturn, useMergeRefs } from '@wordpress/compose';
...
function computeAnchorRect(anchorRefFallback, anchorRect, getAnchorRect) {
  ...
  if (getAnchorRect) {
    ...
    return offsetIframe(getAnchorRect(anchorRefFallback.current), anchorRefFallback.current.ownerDocument);
  }
  ...
  if (anchorRef !== false) {
    ...
    if (typeof (anchorRef === null || anchorRef === void 0 ? void 0 : anchorRef.cloneRange) === 'function') {
      return offsetIframe(getRectangleFromRange(anchorRef), anchorRef.endContainer.ownerDocument);
...
var Popover = function Popover(_ref) {
  ...
  var focusOutsideProps = useFocusOutside(handleOnFocusOutside);
  var mergedRefs = useMergeRefs([containerRef, focusOnMount ? constrainedTabbingRef : null, focusOnMount ? focusReturnRef : null, focusOnMount ? focusOnMountRef : null]); // Event handlers
  ...
  var animateClassName = Boolean(animate && animateOrigin) && getAnimateClassName({
    type: 'appear',
