...
import { add, is, mergeEventHandlers, normalizeArrowKey, roundClampString, subtract, useControlledValue, usePropRef } from '@wp-g2/utils';
...
function useCommitValue(_ref) {
  ...
  var resetCommitValue = useCallback(function () {
...
function useFocusedState(_ref3) {
  ...
  var _useState5 = useState(isFocusedProp),
...
function useKeyboardHandlers(_ref6) {
  ...
  var keyboardHandlers = useMemo(function () {
    return {
...
function useNumberActions(_ref7) {
  ...
  var isValueNumeric = is.numeric(value);
  ...
  jumpStep) {
    ...
    var baseValue = is.numeric(value) ? value : 0;
    var nextValue = add(jumpStep * step, shiftStep);
    var next = roundClampString(add(baseValue, nextValue), min, max, shiftStep);
    onChange(next);
  ...
  jumpStep) {
    ...
    var baseValue = is.numeric(value) ? value : 0;
    var nextValue = add(jumpStep * step, shiftStep);
    var next = roundClampString(subtract(baseValue, nextValue), min, max, shiftStep);
    onChange(next);
  }, [skipAction, propRefs, step]);
  return {
    increment: increment,
...
export function useTextInputState(props) {
  var defaultValue = props.defaultValue,
      ...
      onChangeProp = _props$onChange === void 0 ? noop : _props$onChange,
      ...
      type = props.type,
      format = props.format,
      otherProps = _objectWithoutProperties(props, ["defaultValue", "dragAxis", "incrementFromNonNumericValue", "isCommitOnBlurOrEnter", "isFocused", "onChange", "value", "min", "max", "step", "validate", "isShiftStepEnabled", "shiftStep", "type", "format"]);
  ...
  var _useCommitValue = useCommitValue({
    value: value
      ...
      _useCommitValue2 = _slicedToArray(_useCommitValue, 3),
      commitValue = _useCommitValue2[0],
      setCommitValue = _useCommitValue2[1],
      resetCommitValue = _useCommitValue2[2];

  var inputValue = isNil(commitValue) ? value : commitValue;

  var _useFocusedState = useFocusedState({
    value: isFocusedProp
  }),
      _useFocusedState2 = _slicedToArray(_useFocusedState, 2),
      isFocused = _useFocusedState2[0],
      setFocused = _useFocusedState2[1];

  var handleOnCommit = useCallback(function (next) {
    var isValid = true;
  ...
  var dragHandlersRef = usePropRef(dragHandlers);
  ...
  var keyboardHandlers = usePropRef(mergeEventHandlers(baseKeyboardHandlers, numberKeyboardHandlers)).current;
